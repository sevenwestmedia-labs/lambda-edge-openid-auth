...
import { DiscoveryDocument } from '../lib/idps/discovery-document'

// no types available for supertest-session :(
...
/**
 * Replace the root urls in the discovery doc with that of the issuer since
 * node-oidc-provider uses the request url generated by supertest as the root url
 */
function fixupDiscoveryDoc(brokenDoc: any) {
    const serverUrl = brokenDoc.issuer
    return Object.entries(brokenDoc).reduce(
        (doc, [key, value]) => ({
            ...doc,
            [key]:
                typeof value === 'string'
...
export async function oidcServer(redirectUris: string[]): Promise<OIDCServer> {
    ...
    const discoveryDoc = fixupDiscoveryDoc(
        (
            await supertest(app)
